...
const RecurringTaskDetail = () => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 p-2 sm:p-3 lg:p-4">
      <motion.div
        className="max-w-3xl mx-auto mt-2 p-4 sm:p-6 bg-white rounded-3xl shadow-2xl border border-blue-200 relative overflow-hidden"
        initial="hidden"
        animate="visible"
      ...
      >
        ...
        {/* Task Details Card */}
        <motion.div
          className={`bg-white border-l-4 ${getPriorityBorderColor(task.taskPriority)} p-4 rounded-lg shadow-lg mb-6 transition-all duration-300`}
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-xs text-gray-600">
            <p className="flex items-center gap-2">Assigned To: <span className="font-medium text-blue-700">{task.taskAssignedTo?.email || 'N/A'}</span></p>
            <p className="flex items-center gap-2">Assigned By: <span className="font-medium text-blue-700">{task.taskAssignedBy?.email || 'N/A'}</span></p>
            <p className="flex items-center gap-2">Start Date: {new Date(task.taskStartDate).toLocaleDateString()}</p>
            {task.taskEndDate && (
              <p className="flex items-center gap-2">End Date: {new Date(task.taskEndDate).toLocaleDateString()}</p>
            )}
            <p className="flex items-center gap-2">Priority: <span className={`font-semibold ${task.taskPriority === 'High' ? 'text-red-500' : task.taskPriority === 'Medium' ? 'text-yellow-600' : 'text-green-600'}`}>{task.taskPriority}</span></p>
